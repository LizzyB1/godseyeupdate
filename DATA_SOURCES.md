...
| **GEBCO 2020 via Open Topo Data** | Undersea depth contour lines (self-computed via marching squares) and point depth-marker readouts in the Bathymetry box | GEBCO grid: public domain; Open Topo Data: free public API, no key, rate-limited (1 req/s, 100 locations/request, 1000/day) | "Depth contours & markers: GEBCO 2020 grid via Open Topo Data (gebco.net, public domain)" |
...
- **Bathymetry (GEBCO via Open Topo Data — contours and depth markers).** Both toggles in the Bathymetry box are off by default and keyless — no signup, no `.env` entry. `src/data/bathymetry.js` fetches directly from the browser (no server-side proxy, unlike TomTom/FIRMS/etc.): each recompute cycle samples one shared grid of points (up to 90, under Open Topo Data's 100-locations-per-request cap) across the current view from Open Topo Data's `gebco2020` endpoint. Depth markers render a subsample of that grid directly; depth contours (isobaths) are computed locally from the same grid with marching squares (`src/data/contourMath.js`, the same pure logic the land-elevation contour system already uses) rather than fetched pre-drawn — an earlier version queried NOAA's `OceanReports/BathymetricContours` ArcGIS layer for pre-drawn contours, but that layer is scoped to U.S. waters only and returned empty for open ocean elsewhere, so it was dropped in favor of this self-computed, globally-working approach. Every depth lookup is cached indefinitely in the shared client-side `apiCache` (IndexedDB) keyed by rounded lat/lon and shared between both layers, and requests to Open Topo Data are throttled to stay under its published free-tier ceiling (1 request/second). The credit registers dynamically the moment either toggle is switched on, same pattern as TomTom.
