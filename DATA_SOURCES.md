...
| **NOAA OceanReports Bathymetric Contours** (ArcGIS REST) | Undersea depth contour lines (isobaths) in the Bathymetry box | Public NOAA data, no key/registration | "Undersea depth contours: NOAA — OceanReports Bathymetric Contours" |
| **GEBCO 2020 via Open Topo Data** | Point depth-marker readouts in the Bathymetry box | GEBCO grid: public domain; Open Topo Data: free public API, no key, rate-limited (1 req/s, 100 locations/request, 1000/day) | "Depth markers: GEBCO 2020 grid via Open Topo Data (gebco.net, public domain)" |
...
- **Bathymetry (NOAA contours + GEBCO/Open Topo Data depth markers).** Both toggles in the Bathymetry box are off by default and keyless — no signup, no `.env` entry. `src/data/bathymetry.js` fetches directly from the browser (no server-side proxy, unlike TomTom/FIRMS/etc.): NOAA's `OceanReports/BathymetricContours` ArcGIS layer is queried by the current view's bounding box for isobath polylines; depth markers query Open Topo Data's `gebco2020` endpoint for a small grid of points per view. Every depth lookup is cached indefinitely in the shared client-side `apiCache` (IndexedDB) keyed by rounded lat/lon, and requests to Open Topo Data are throttled to stay under its published free-tier ceiling (1 request/second). Both credits register dynamically the moment their toggle is switched on, same pattern as TomTom.
